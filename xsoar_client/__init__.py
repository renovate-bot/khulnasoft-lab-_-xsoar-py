...
import xsoar_client.xsoar_api as xsoar_api
import six
import os
import datetime
import tzlocal
...
from xsoar_client.xsoar_api import ApiClient
from xsoar_client.xsoar_api.configuration import Configuration
...
XSOAR_HTTP_HEADERS_REGEX_PATTERN = r'^([\w-]+=[^=,\n]+)(,[\w-]+=[^=,\n]+)*$'
...
def configure(base_url=None, api_key=None, advanced_api_key=None, verify_ssl=None, proxy=None, username=None, password=None,
    ...
    * XSOAR_CONNECTION_POOL_MAXSIZE (specify a connection pool max size)
    * HTTP_PROXY or HTTPS_PROXY (If you are using a proxy)

    :param base_url: str - Base url of your Xsoar instance.
    ...
    """
    base_url = base_url or os.getenv('XSOAR_BASE_URL')
    if not base_url:
        raise RuntimeError('the XSOAR_BASE_URL value is not set.')
    api_key = api_key or os.getenv('XSOAR_API_KEY')
    auth_id = auth_id or os.getenv('XSOAR_API_KEY_ID') or os.getenv('XSIAM_AUTH_ID')
    username = username or os.getenv('XSOAR_USERNAME')
    password = password or os.getenv('XSOAR_PASSWORD')
    advanced_api_key = advanced_api_key or os.getenv('XSOAR_ADVANCED_API_KEY')

    if additional_headers is None:
        if headers := os.getenv('XSOAR_HTTP_HEADERS'):
            if re.match(r'^ *$', headers):  # catch any case of empty string
                additional_headers = {}
            elif re.match(XSOAR_HTTP_HEADERS_REGEX_PATTERN, headers):
    ...
    if verify_ssl is None:
        verify_env = os.getenv('XSOAR_VERIFY_SSL')
        if verify_env:
    ...
    if connection_pool_maxsize is None:
        connection_pool_maxsize = os.getenv('XSOAR_CONNECTION_POOL_MAXSIZE')
        if connection_pool_maxsize:
            if not connection_pool_maxsize.isdigit():
                err_msg = ('XSOAR_CONNECTION_POOL_MAXSIZE env variable should be set to a number'
                           f' but instead received "{connection_pool_maxsize}"')
                raise ValueError(err_msg)
            else:
                connection_pool_maxsize = int(connection_pool_maxsize)
    if not base_url:
        raise ValueError('You must specify base_url either as a parameter or via env variable: XSOAR_BASE_URL')
    ...
    if auth_id and not (api_key or advanced_api_key):
        raise ValueError('You must specify either api_key or advanced_api_key or use env variable XSOAR_API_KEY or XSOAR_ADVANCED_API_KEY ' 
                         'to use Cortex XSIAM api or XSOAR 8+, or remove the auth_id and/or the env variable XSOAR_API_KEY_ID '
    ...
    if api_key or auth_id:
        ...
        api_client.user_agent = 'xsoar-py/' + __version__
        api_client.default_headers.update(additional_headers)
        api_instance = xsoar_api.DefaultApi(api_client)
        return api_instance
    else:
...
def login(
    ...
    configuration_orig.host = base_url or os.getenv('XSOAR_BASE_URL', None)
    ...
    configuration = Configuration()
    configuration.host = base_url or os.getenv('XSOAR_BASE_URL', None)
    ...
    api_client.user_agent = 'xsoar-py/' + __version__
    api_client.default_headers.update(additional_headers)
    mid_client = xsoar_api.DefaultApi(api_client)
    ...
    mid_api_client.user_agent = 'xsoar-py/' + __version__
    mid_api_client.default_headers.update(additional_headers)
    final_api_client = xsoar_api.DefaultApi(mid_api_client)
...
def generic_request_func(self, path, method, body=None, **kwargs):
    """
    This method allows for generic requests to be made against the Xsoar server to endpoints that
...
def get_layouts_url_for_demisto_version(api_client, params):
    """
    This function returns the correct url to upload a layout according to Xsoar version that is being used.
